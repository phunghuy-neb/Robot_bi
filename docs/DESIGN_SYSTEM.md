...
### Content-width pattern — tab Học tập (spec 007)

App shell vẫn dùng 1 breakpoint `768px` (nav). Bên trong **tab Học tập**, nội dung áp mẫu **rộng khi duyệt / hẹp khi làm bài**:

| Lớp | Class | max-width | Dùng cho |
|---|---|---|---|
| Duyệt | `.learn-browse` | `1280px` căn giữa | Lưới môn, trang chi tiết môn (thẻ chế độ) |
| Làm bài | `.learn-quiz` | `640px` căn giữa | Luyện theo bài / sổ lỗi (đọc dễ, 1 cột) |
| Luồng cũ (Lộ trình/đề) | inline | `560px` căn giữa | View module/đề tái dùng (≤640 cap) |

Breakpoint phụ (chỉ trong Học tập): `360px` (lưới môn 2 cột min 132px) · `1024px` (`.subject-detail-grid` tách `2fr 1fr`, dưới đó 1 cột). Lưới môn `repeat(auto-fill, minmax(140px,1fr))`. Mục tiêu: không cuộn ngang 320→1920, không "co giữa" mobile, không kéo căng ultra-wide.

**Mastery theo chủ đề (IXL SmartScore-style)** — màu **luôn kèm chữ** (màu không phải tín hiệu duy nhất, a11y):

| Điểm | Nhãn | Class |
|---|---|---|
| < 60 | Cần cố gắng | `.mastery-low` |
| 60–79 | Khá | `.mastery-mid` |
| 80–89 | Thạo | `.mastery-good` |
| ≥ 90 | Làm chủ | `.mastery-master` |

Chế độ chưa sẵn sàng (vd Lộ trình môn chưa có nội dung): `.mode-card.coming-soon` = badge "Sắp có" + viền nét đứt + dáng mờ + chữ "Đang xây dựng" (trạng thái thể hiện bằng badge+chữ, không chỉ màu).
