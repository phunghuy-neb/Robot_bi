...
- **Learning Hub Phase 3 — HSG/exam packs (strategy: one subject deep at a time):**
  ...
  - DONE (uncommitted, 2026-06-24): `resources/learning/geography_exam.json` — Địa lý, 6 papers / 42 questions (subject='geography'; comp_level set for hsg_*). Địa lý VN + thế giới: vị trí/hình chữ S, châu lục/đại dương, đồng bằng/sông, gió mùa, Phan-xi-păng, bô-xít, El Nino, chí tuyến, đô thị hóa… Validated: 0 bad answers, unique paper_ids. Aggregate now: 24 subjects / 148 papers / 1082 questions / 0 invalid. **Ready to commit.**
  - **NEXT STEP (đang chạy tự động sau 07:15)**: làm tiếp GDCD (civics) → các môn khác còn lại (informatics, programming, music, art, economics, health, life_skills, logic, chinese, japanese, korean) nếu còn quota. Cùng pattern 6 đề/42 câu, 6 track. Helper build dùng chung ở scratchpad (`build_exam_common.py`).
