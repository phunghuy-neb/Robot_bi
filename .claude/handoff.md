...
- **Learning Hub Phase 3 — HSG/exam packs (strategy: one subject deep at a time):**
  ...
  - DONE (uncommitted, 2026-06-24): `resources/learning/history_exam.json` — Lịch sử, 6 papers / 42 questions (subject='history'; comp_level set for hsg_*). Lịch sử VN: Văn Lang→Bạch Đằng→Lý/Trần/Lê→Quang Trung→Pháp xâm lược→Đảng 1930→1945/1954/1975→Đổi mới→ASEAN/WTO; HSG có Hình thư, Diên Hồng, Đại Việt sử ký… Validated: 0 bad answers, unique paper_ids. Aggregate now: 24 subjects / 142 papers / 1040 questions / 0 invalid. **Ready to commit.**
  - **NEXT STEP (đang chạy tự động sau 07:15)**: làm tiếp Địa lý → GDCD → (các môn khác). Cùng pattern 6 đề/42 câu, 6 track. Helper build dùng chung ở scratchpad (`build_exam_common.py`).
