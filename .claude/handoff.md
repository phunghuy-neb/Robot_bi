...
- **Learning Hub Phase 3 — HSG/exam packs (strategy: one subject deep at a time):**
  ...
  - DONE (uncommitted, 2026-06-24): `resources/learning/english_exam.json` — Tiếng Anh, 6 papers / 42 questions (same 6 tracks; subject='en'; comp_level set for hsg_*). MCQ ngữ pháp/từ vựng có question_vi + giải thích tiếng Việt (tenses, conditionals, passive, đảo ngữ, phrasal verbs, idioms… từ tiểu học → HSG). Validated: 0 bad answers, unique paper_ids. Aggregate all packs now: 24 subjects / 112 papers / 830 questions / 0 invalid. Full `run_tests.py` not runnable here (missing chromadb/dotenv); validated by replicating `_seed_learning_packs` logic. **Ready to commit.**
  - **NEXT STEP**: 3 môn lõi (Toán/Tiếng Việt/Ngữ văn/Tiếng Anh) đã có gói exam/HSG. Tiếp theo (tùy chọn): các môn còn lại (Khoa học/Lý/Hóa/Sinh/Sử/Địa…) theo cùng pattern, hoặc `toeic_sw` (cần STT/chấm tự luận). Hỏi user môn nào nếu chưa rõ.
