...
- **Learning Hub Phase 3 — HSG/exam packs (strategy: one subject deep at a time):**
  ...
  - DONE (uncommitted, 2026-06-24): `resources/learning/literature_exam.json` — Ngữ văn, 6 papers / 42 questions (same 6 tracks; subject='literature'; comp_level set for hsg_*). Hand-authored văn học MCQ (tác giả–tác phẩm, thể loại, nội dung/nghệ thuật, giá trị nhân đạo, phong cách, từ dân gian tiểu học → THPT/HSG). Validated: 0 bad answers, unique paper_ids. Aggregate all packs now: 24 subjects / 106 papers / 788 questions / 0 invalid. Full `run_tests.py` not runnable here (missing chromadb/dotenv); validated by replicating `_seed_learning_packs` logic. **Ready to commit.**
  - **NEXT STEP**: next core subject deeply — Tiếng Anh (english_exam.json), entrance lớp 6/10 + HSG levels. Same pattern as math/vietnamese/literature.
