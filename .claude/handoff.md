...
- **Learning Hub Phase 3 — HSG/exam packs (strategy: one subject deep at a time):**
  ...
  - DONE (uncommitted, 2026-06-24): `resources/learning/science_exam.json` — Khoa học, 6 papers / 42 questions (same 6 tracks; subject='science'; comp_level set for hsg_*). KHTN tổng hợp (lý/hóa/sinh/môi trường: quang hợp, trọng lực, tế bào, biến đổi hóa học, ti thể, ADN, bảo toàn năng lượng…). Validated: 0 bad answers, unique paper_ids. Aggregate now: 24 subjects / 118 papers / 872 questions / 0 invalid. **Ready to commit.**
  - **NEXT STEP (đang chạy tự động sau 07:15)**: làm tiếp các môn còn lại theo thứ tự Vật lý → Hóa học → Sinh học → Lịch sử → Địa lý → GDCD → (các môn khác). Cùng pattern 6 đề/42 câu, 6 track. Helper build dùng chung ở scratchpad (`build_exam_common.py`).
