...
- 2026-06-24: **Learning Hub Phase 2 — content packs + batch AI generation** (branch `003-web-search-integration`):
  - `resources/learning/*.json` (NEW, 16 packs): real curriculum-accurate MCQ content authored by parallel agents (English, Math, Science/Physics/Chemistry/Biology, IELTS, TOEIC L&R, Tiếng Việt, Ngữ văn, History/Geography/Civics, Chinese/Japanese/Korean). Total **64 exam papers / ~494 questions / 16 subjects**. Every `answer` is exactly one of its `options` (validated). biology/geography/civics packs were hand-authored by the main agent after the content sub-agents hit the session limit.
  - `src/infrastructure/database/db.py`: `_seed_learning_packs(conn)` loads all `resources/learning/*.json` into `question_bank` (status='published', source='pack') + assembles `exam_papers`; skips any question whose answer ∉ options; idempotent. Called from `init_db()` after `_seed_exam_content`. **Pack JSON schema is documented in the function docstring.** Total published after seed: 16 subjects / 67 papers / 509 questions.
  ...
  - `tests/run_tests.py`: Group 80 (6 tests) — pack load counts, published-answer integrity, subject coverage, curriculum endpoint, pack-exam submit end-to-end, batch generate + auth + size guard.
  - **How to add content**: drop a new `resources/learning/<subject>.json` following the schema → it auto-seeds on next `init_db()`. The AI pipeline (`generate` / `generate-batch`) writes to `question_bank` status='review' → admin publishes. To convert AI-published questions into a versioned pack, export them to a JSON pack later.
  - **2026-06-24 follow-up (same branch)**: added 8 more subject packs — `informatics`, `programming`, `music`, `art`, `economics`, `health`, `life_skills`, `logic` (each 3 exams; +24 papers / +168 questions). Added their SUBJECT_LABELS + CURRICULUM_BLUEPRINT entries in `exam_router.py`. Total published now: **24 subjects / 91 papers / 677 questions / 0 invalid answers**. Group 80 thresholds bumped (≥85 papers, ≥640 questions, ≥22 subjects) and required-subject list extended.
  - **NOTE**: still not packed: `toeic_sw` (Speaking/Writing — Phase 3 territory, needs free-text/STT grading) and dedicated HSG/exam-track papers (`hsg_*`, `exam_grade6/10`). Produce these via the batch-generate pipeline (needs LLM keys) or more authoring. Curriculum blueprint lists the topics.

- 2026-06-24: **Learning Hub Phase 1 — exam system + AI question-bank pipeline** (branch `003-web-search-integration`, commit `a0943a9`):
  - `src/infrastructure/database/db.py`: Added 4 tables in `init_db()` — `question_bank`, `exam_papers`, `exam_paper_questions`, `exam_sessions` (+ indexes). New `_seed_exam_content(conn)` is idempotent (`INSERT OR IGNORE`) and seeds 3 starter papers (TOEIC L&R, IELTS Reading, Toán THPT = 15 questions). Starter questions are authored with the correct answer as `options[0]`.
  - `src/api/routers/exam_router.py` (NEW): `GET /api/learning/subjects`, `/api/learning/tracks` (11 tracks: practice / hsg_school|district|province|national / exam_grade6|grade10|thpt / ielts / toeic_lr / toeic_sw); `GET /api/learning/exams` (filter subject/track/level/skill/age_group), `GET /api/learning/exams/{paper_id}` (**answers + explanations hidden**), `POST /api/learning/exams/{paper_id}/submit` (auto-grade MCQ, store `exam_sessions`, return report w/ explanations + pass/fail), `GET /api/learning/exams/sessions[/{id}]`. Admin (is_admin via `require_admin`): `POST /api/learning/admin/generate` (LLM via `stream_chat`, honors `SKIP_LLM` env → deterministic offline stub; inserts to `question_bank` status='review'), `GET /api/learning/admin/review`, `POST /api/learning/admin/review/{question_id}` (publish/reject + edits), `POST /api/learning/admin/exams` (assemble paper from question_ids or auto-select).
  ...
  - `frontend/parent_app/src/services/api.js`: added `getExamSubjects/getExamTracks/getExams/getExam/submitExam/getExamSessions/getExamSession` + admin `adminGenerateQuestions/adminReviewQueue/adminReviewQuestion/adminAssembleExam`.
  - `frontend/parent_app/src/pages/LearningHubPage.jsx`: added `ModeToggle` (📚 Học theo chủ đề / 📝 Làm đề & Thi thử). Exam mode views: track catalog (grouped by kind) → exam list (best %/attempts) → playing (countdown timer + question-nav dots + change answers) → graded result (per-question review w/ explanations, pass/fail). Vite build OK.
  - `tests/run_tests.py`: added Group 79 (9 tests). Full suite **630/631 PASS**; the 1 failure is the pre-existing flaky Group-64 real-API LLM stress test (`safety filter pass tat ca`, threshold ≤1, got 2) — unrelated to exam code.
  - **Scope**: Phase 1 = infrastructure + small starter content. Bulk content (37 subjects, HSG/exam/IELTS/TOEIC, ~426k questions target) is produced later by the AI generation pipeline + admin review (Phase 2). Phase 3 = IELTS/TOEIC Speaking via Robot Bi STT. User approved the full subject catalog across ages 3–18 incl. Chinese/Japanese/Korean + Ngữ văn.
