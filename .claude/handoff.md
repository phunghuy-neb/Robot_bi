...
- **Learning Hub Phase 3 — HSG/exam packs (strategy: one subject deep at a time):**
  ...
  - DONE (uncommitted, 2026-06-24): `resources/learning/physics_exam.json` — Vật lý, 6 papers / 42 questions (subject='physics'; comp_level set for hsg_*). Đo lường, vận tốc, định luật Ôm/Niu-tơn, Ác-si-mét, dao động, sóng âm, giao thoa, hạt nhân… Validated: 0 bad answers, unique paper_ids. Aggregate now: 24 subjects / 124 papers / 914 questions / 0 invalid. **Ready to commit.**
  - **NEXT STEP (đang chạy tự động sau 07:15)**: làm tiếp Hóa học → Sinh học → Lịch sử → Địa lý → GDCD → (các môn khác). Cùng pattern 6 đề/42 câu, 6 track. Helper build dùng chung ở scratchpad (`build_exam_common.py`).
