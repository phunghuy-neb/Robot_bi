...
- **Learning Hub Phase 3 — HSG/exam packs (strategy: one subject deep at a time):**
  ...
  - DONE (uncommitted, 2026-06-24): `resources/learning/biology_exam.json` — Sinh học, 6 papers / 42 questions (subject='biology'; comp_level set for hsg_*). Cơ thể người/thực vật, tế bào/lục lạp, Mendel/di truyền, ADN/dịch mã, nguyên-giảm phân, hệ sinh thái, ổ sinh thái… Validated: 0 bad answers, unique paper_ids. Aggregate now: 24 subjects / 136 papers / 998 questions / 0 invalid. **Ready to commit.**
  - **NEXT STEP (đang chạy tự động sau 07:15)**: làm tiếp Lịch sử → Địa lý → GDCD → (các môn khác). Cùng pattern 6 đề/42 câu, 6 track. Helper build dùng chung ở scratchpad (`build_exam_common.py`).
