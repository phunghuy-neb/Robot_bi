...
- **Learning Hub Phase 3 — HSG/exam packs (strategy: one subject deep at a time):**
  ...
  - DONE (uncommitted, 2026-06-24): `resources/learning/chemistry_exam.json` — Hóa học, 6 papers / 42 questions (subject='chemistry'; comp_level set for hsg_*). Thể/hỗn hợp, kí hiệu/CTHH, axit-bazơ-quỳ tím, bảo toàn khối lượng, đồng vị, mol/Avogadro, oxit, lưỡng tính, este hóa… Validated: 0 bad answers, unique paper_ids. Aggregate now: 24 subjects / 130 papers / 956 questions / 0 invalid. **Ready to commit.**
  - **NEXT STEP (đang chạy tự động sau 07:15)**: làm tiếp Sinh học → Lịch sử → Địa lý → GDCD → (các môn khác). Cùng pattern 6 đề/42 câu, 6 track. Helper build dùng chung ở scratchpad (`build_exam_common.py`).
