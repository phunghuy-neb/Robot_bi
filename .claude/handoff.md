...
- **Learning Hub Phase 3 — HSG/exam packs (strategy: one subject deep at a time):**
  ...
  - DONE (uncommitted, 2026-06-24): `resources/learning/vietnamese_exam.json` — Tiếng Việt, 6 papers / 42 questions (same 6 tracks; subject='vietnamese'; comp_level set for hsg_*). Hand-authored language-focused MCQ (chính tả, từ loại, từ láy/ghép, biện pháp tu từ, thành phần câu, phong cách ngôn ngữ, hàm ý…). Validated: 0 bad answers, unique paper_ids. Aggregate all packs now: 24 subjects / 100 papers / 746 questions / 0 invalid. Full `run_tests.py` not runnable here (missing chromadb/dotenv); validated by replicating `_seed_learning_packs` logic. **Ready to commit.**
  - **NEXT STEP**: next core subject deeply — Ngữ văn (literature_exam.json) then Tiếng Anh (english_exam.json), each entrance lớp 6/10 + HSG levels. Same pattern as math/vietnamese.
